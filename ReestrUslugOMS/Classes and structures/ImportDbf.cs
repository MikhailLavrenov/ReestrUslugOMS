...
{
    /// <summary>
    /// Выполняет импорт данных из dbf файлов в sql таблицы. Записывает результат выполнения в sql таблицу.
    /// </summary>
    class ImportDbf
    {
        /// <summary>
        /// Список объектов, подлежащих импорту.
        /// </summary>
        ...
        /// <summary>
        /// Выполняет импорт всех объектов из списка. Результат выполнения записывает в таблицу.
        /// </summary>
        public void Import()
        {
        ...
        /// <returns>Имя созданной временной таблицы.</returns>
        ...
        /// </summary>
        /// <param name="mainTable">Имя основной таблицы.</param>
        /// <param name="tempTable">Имя временной таблицы.</param>
        /// <param name="period">Период, если нет - null.</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Выполняет импорт данных из dbf файла в существующую временную таблицу sql сервера.
        /// </summary>
        public class Item
        {
            /// <summary>
            ...
            /// <param name="sqlTable">Имя sql таблицы</param>
            /// <param name="period">Период импорта. Если нет - null.</param>
            /// <param name="organisation">Название страховой компании, если нет - null.</param>
            ...
            private DataTable GetSqlData()
            {
                ...
                using (var tab = NDbfReader.Table.Open(strm))
                {
                    ...
                    var dbfFields = tab.Columns.Select(x => new { Column = x, Name = Tools.DropNullCharacter(x.Name), Type = Nullable.GetUnderlyingType(x.Type) ?? x.Type }).ToList();
            ...
            /// <summary>
            /// Импортирует данные во временную таблицу sql сервера.
            /// </summary>
            /// <param name="tempSqlTable">Имя временной таблицы</param>
